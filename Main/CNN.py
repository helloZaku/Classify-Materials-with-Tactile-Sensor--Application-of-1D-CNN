...
x16 = tf.keras.layers.Flatten()(z3)
x17 = tf.keras.layers.Dense(128, activation = 'relu')(x16)
x18 = tf.keras.layers.Dense(3, activation = 'softmax')(x17)

# model execution
model = tf.keras.Model(inputs = input_signal, outputs = x18)
model.summary()
adam = Adam(learning_rate = 0.00001,beta_1 = 0.9, beta_2=0.999, epsilon = 1e-07, decay=0.0,amsgrad=False)
...
train_history = model.fit(X_train,y_train,epochs = 50, batch_size=16,validation_split=0.2,shuffle=True)

# test model
model.evaluate(X_test,y_test)

#save model
model.save('D:\Jupyter\CNN\Test\Saved_Models\CNNCopy.h5')
...
#Save val acc and prob
pd.DataFrame(y4).to_csv("D:\Jupyter\CNN\Test\Output\Val_Acc.csv")